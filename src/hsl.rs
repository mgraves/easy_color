...
/*impl From<(u32, u32, u32)> for HSL {
  fn from(value: (u32, u32, u32)) -> Self {
    if !(0..=360).contains(&value.0)
      || !(0..=100).contains(&value.1)
      || !(0..=100).contains(&value.2)
    {
      let error_string = format!("HSL: args ({},{},{}) value error, first value must between 0~360, others must between 0~100!", value.0, value.1, value.2);
      panic!("{error_string}");
    } else {
      Self {
        h: value.0,
...
impl HSL {
  ...
  pub fn new_unchecked(hue: u32, saturation: u32, lightness: u32) -> Self {
    Self {
      h: hue,
      s: saturation,
      l: lightness,
